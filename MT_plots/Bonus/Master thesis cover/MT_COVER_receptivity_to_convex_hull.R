...
set.seed(1)
n.draws <- 500 # Number of stochastic process drawn
...
if(print == T){pdf("MT_COVER_receptivity_to_convex_hull.pdf", width = 3.5/2*3, height = 3)}

par(mfrow = c(1,1), mar = c(0,0,0,0) + 0.1, cex = 1.0)

rainbow.plot(smooth.gp.data.new.alpha.volume, seq(0,10, length = 200), col = col, ylab="", xlab="", main="", xaxt="n", yaxt="n", ylim = range(rbind(smooth.gp.data.new,smooth.gp.data)))
